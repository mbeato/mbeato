#!/usr/bin/env node
// Generates assets/services.svg — a systemctl-style status panel for active
// projects, with per-unit numbers pulled live from the GitHub GraphQL API.
// Unit list, blurbs, and the modules line live in services.json.
//
//   GITHUB_TOKEN=... node scripts/generate-services.js
...
async function main() {
  ...
  <!-- Generated by scripts/generate-services.js — data from the GitHub GraphQL API. Edit services.json, not this file. -->
  <rect width="900" height="${height}" fill="${C.bg}"/>

  <text x="40" y="34" font-size="11" letter-spacing="2" fill="${C.mid}">◆ SERVICES</text>
  <text x="118" y="34" font-size="11" letter-spacing="2" fill="${C.dim}"> — LIVE FROM THE GITHUB API</text>
  <text x="860" y="34" text-anchor="end" font-size="11" letter-spacing="2" fill="${C.dim}">UPDATED ${updated} · DAILY</text>
...
`;
  ...
  const out = join(__dirname, "..", "assets", "services.svg");
  writeFileSync(out, svg);
  console.log(`wrote ${out} (${height}px tall, ${config.units.length} units)`);
}
